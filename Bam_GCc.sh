...
cwd=`pwd`;
nb_cores=4;
OUT="$PWD/DATA/GC_correction_4Bams_peaksExcluded"
OUT_Bam="$PWD/DATA/BAMs_All_GCc"
OUT_BW="$PWD/DATA/BigWigs_All"

DIR_Bam="$PWD/DATA/BAMs_All"
...
DIR_peak="$PWD/DATA/calledPeaks/macs2_broad"
...
do
	...
	if [ ! -e ${OUT_BW}/${cc}.bw ]; then
	    echo -e "module unload deeptools; module load python/2.7.3; module load pysam/0.8.3; module load deeptools/2.2.3-python2.7.3;
             bamCoverage -b $b -o ${OUT_BW}/${cc}.bw --outFileFormat=bigwig --normalizeUsingRPKM;" >> $sscript;
	fi;
	
	if [ ! -e ${OUT_BW}/${cc2}.bw ]; then 
	    echo -e "bamCoverage -b ${OUT_Bam}/${cc2}.bam -o ${OUT_BW}/${cc2}.bw --outFileFormat=bigwig --normalizeUsingRPKM; " >> $sscript
	fi;
